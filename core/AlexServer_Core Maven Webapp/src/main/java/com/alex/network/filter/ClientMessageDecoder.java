	...
	public boolean doDecode(IoSession iosession, IoBuffer iobuffer,
      ...
      private Object mappingMessageObject(String  message) throws Exception {
		...
		if (name.equals("message")) {
			...
			body.setReceiver(doc.getElementsByTagName("niname").item(0).getTextContent());
			body.setFormat(doc.getElementsByTagName("headimg").item(0).getTextContent()); 
			body.setMid(doc.getElementsByTagName("usercustomerid").item(0).getTextContent());
			body.setReceiver_headimg(doc.getElementsByTagName("receiver_headimg").item(0).getTextContent());
			body.setReceiver_niname(doc.getElementsByTagName("receiver_niname").item(0).getTextContent());
